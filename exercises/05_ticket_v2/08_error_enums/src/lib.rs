...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    return match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(error) => match error {
            TicketNewError::TitlesError(error) => panic!("{error}"),
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitlesError("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return  Err(TicketNewError::TitlesError("Title cannot be longer than 50 characters".to_string()));
